...
Status
MikolovRnn::init(
    const MikolovRnnModelHeader& header,
    const util::ArraySlice<float> &weights, const util::ArraySlice<float> &maxentW) {
...
Status MikolovModelReader::open(StringPiece filename) {
  ...
  JPP_RETURN_IF_ERROR(data_->rnnDictionary.open(filename, util::MMapType::ReadOnly));
  auto nnetFile = filename.str() + ".nnet";
  JPP_RETURN_IF_ERROR(data_->rnnModel.open(nnetFile, util::MMapType::ReadOnly));
  JPP_RETURN_IF_ERROR(data_->rnnDictionary.map(&data_->dictFrag, 0, data_->rnnDictionary.size()));
  JPP_RETURN_IF_ERROR(data_->rnnModel.map(&data_->modelFrag, 0, data_->rnnModel.size()));
  return Status::Ok();
}
...
Status allocAligned(std::unique_ptr<float, FreeDeleter<float>>& result, size_t size) {
  float* ptr;
...
Status copyArray(StringPiece data, std::unique_ptr<float, FreeDeleter<float>>& result, size_t size, size_t* offset) {
  auto fullSize = size * sizeof(float);
  if (*offset + fullSize > data.size()) {
    return Status::InvalidParameter() << "can't copy rnn weight data, sizes are wrong";
  }
...
Status MikolovModelReader::parse() {
  ...
  JPP_RETURN_IF_ERROR(ssvReader.initFromMemory(data_->dictFrag.asStringPiece()));
  ...
  JPP_RETURN_IF_ERROR(allocAligned(data_->embeddingData, hdr.layerSize * hdr.vocabSize));
  JPP_RETURN_IF_ERROR(allocAligned(data_->nceEmbeddingData, hdr.layerSize * hdr.vocabSize));
  JPP_RETURN_IF_ERROR(allocAligned(data_->matrixData, hdr.layerSize * hdr.layerSize));
  JPP_RETURN_IF_ERROR(allocAligned(data_->maxentWeightData, hdr.maxentSize));
  size_t start = sizeof(MikolovRnnModePackedlHeader);
  JPP_RETURN_IF_ERROR(copyArray(contents, data_->embeddingData, hdr.layerSize * hdr.vocabSize, &start));
  JPP_RETURN_IF_ERROR(copyArray(contents, data_->nceEmbeddingData,  hdr.layerSize * hdr.vocabSize, &start));
  JPP_RETURN_IF_ERROR(copyArray(contents, data_->matrixData, hdr.layerSize * hdr.layerSize, &start));
  JPP_RETURN_IF_ERROR(copyArray(contents, data_->maxentWeightData, hdr.maxentSize, &start));
