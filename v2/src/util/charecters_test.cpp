...
Status checkByteSequence(std::initializer_list<u8> useq){
  ...
  StringPiece sp(reinterpret_cast<const char *>(malformed_seq.data()), 
    reinterpret_cast<const char *>(malformed_seq.data()+malformed_seq.size()));
...
TEST_CASE("string is processed to character information", "[characters]") {
  ...
  CHECK(s.isOk());  // TODO real test (maybe several)
}
...
TEST_CASE("preprocessRawData collectly treats UTF8 sequence あ", "[characters]") {
  CHECK(checkByteSequence({0xe3,0x81,0x82}));
}

TEST_CASE("preprocessRawData collectly treats malformed UTF8 continuation byte sequences", "[characters]") {
  std::vector<InputCodepoint> result;
...
TEST_CASE("preprocessRawData collectly treats impossible utf8 byte sequences", "[characters]") {
  // Impossible bytes
  // 0xfe
