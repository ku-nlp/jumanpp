...
public:
  ...
  typedef jumanpp::ptrdiff_t difference_type;
...
private:
  ...
  void set_size_internal(size_t n) {
    if (is_inline()) {
      ...
      JPP_DCHECK_EQ(u_.data[kSize - 1], kSentinel);
    }
  }
...
void InlinedVector<T, N>::swap(InlinedVector& other) {
  ...
  JPP_DCHECK_EQ(b->size(), a_size);
  JPP_DCHECK_EQ(a->size(), b_size);
}
