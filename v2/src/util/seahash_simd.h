...
public:
  ...
  JPP_ALWAYS_INLINE u64 result() const noexcept {
    return state_;
  }
...
public:
  ...
  explicit FastHash2(const u64* state) noexcept: state_(_mm_loadu_si128(reinterpret_cast<const __m128i *>(state))) {}
  ...
  JPP_ALWAYS_INLINE void jointMaskStore(FastHash2 other, u32 mask, u32 *result) {
...
public:
  ...
  explicit FastHash4(const u64* state) noexcept: state_(_mm256_loadu_si256(reinterpret_cast<const __m256i *>(state))) {}
  ...
  JPP_ALWAYS_INLINE static __m256i Multiply64Bit(__m256i a, __m256i b)
  {
    ...
    auto ax1_i_ay1_i = _mm256_shuffle_epi32(ax0_ax1_ay0_ay1, _MM_SHUFFLE(3, 3, 1, 1));
    auto bx1_i_by1_i = _mm256_shuffle_epi32(bx0_bx1_by0_by1, _MM_SHUFFLE(3, 3, 1, 1));
    ...
    return _mm256_add_epi64(ax0bx0_ay0by0, _mm256_add_epi64(ax0bx1_ay0by1_32, ax1bx0_ay1by0_32));
  }
  ...
  FastHash4 maskCompact(u32 mask) const noexcept {
    auto v1 = _mm256_permutevar8x32_epi32(state_,
                                          _mm256_set_epi32(0, 2, 4, 6, 0, 2, 4, 6)
    );
  ...
  JPP_ALWAYS_INLINE void joinStore(FastHash4 other, u32 mask, u32 *result) const noexcept {
    auto s = _mm256_blend_epi32(state_, other.state_, JPP_BITS8(0, 0, 0, 0, 1, 1, 1, 1));
    _mm256_storeu_si256(reinterpret_cast<__m256i *>(result), s);
  }

  JPP_ALWAYS_INLINE void jointMaskStore(FastHash4 other, u32 mask, u32 *result) {
    auto v1 = _mm256_permutevar8x32_epi32(state_,
                                          _mm256_set_epi32(0, 2, 4, 6, 0, 2, 4, 6)
    );
    auto v2 = _mm256_permutevar8x32_epi32(other.state_,
                                          _mm256_set_epi32(0, 2, 4, 6, 0, 2, 4, 6))
    ;
