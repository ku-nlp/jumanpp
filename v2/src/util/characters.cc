...
inline CharacterClass getCodeType(char32_t code) noexcept { 
  /* SPACE */
  if (code == 0x20 /* space */ || 
      code == 0x3000 /* IDEOGRAPHIC SPACE (Full-widths space) */ ||
      code == 0x00A0 || code == 0x1680 ||
      code == 0x180E || /*Other spaces on unicode*/
  ...
  /* FIGURE (0-9, ０-９) */ // Special numbers such as ① and ⅷ are treated as symbols.
  else if ((code > 0x2f && code < 0x3a) || (code > 0xff0f && code < 0xff1a)) {
      return CharacterClass::FIGURE;
  }
  /* KANJI_FIGURE (○一七万三九二五亿六八十千四百零, ％, 点余多) */
  else if (code == 0x25cb || //○ Full-widths circle symbol (sometimes it is used instead of zero)
           code == 0x3007 || //〇
           code == 0x96f6 || //零
           code == 0x4e00 || //一
           code == 0x4e8c || //二
      ...
      return CharacterClass::KANJI_FIGURE;
             ...
             code == 0x5E7E || //幾
             false) {
      if (code == 0x6570 || code == 0x4F55 || code == 0x5E7E)
          /* 数，何，幾 */
...
Status preprocessRawData(StringPiece utf8data,
  ...
  while (itr < utf8data.end()){
    ...
    if(!ret)
        return Status::InvalidState() << "Invalid UTF8 sequence";
