...
namespace RNNLM{
    class CRnnLM_dyn:public CRnnLM{
        protected:
            ...
            vector_syn *syn_rec;
            vector_syn* syn_d;		//direct parameters between input and output layer (similar to Maximum Entropy model parameters)
        ...
        public:
            ...
            ~CRnnLM_dyn()		//destructor, deallocates memory
            {
                ...
                if(p_file_syn != nullptr){ delete(p_file_syn);}
                if(p_file_direct != nullptr){ delete(p_file_direct);}
            ...
            void matrixXvector(struct neuron *dest, struct neuron *srcvec, vector_syn *srcmatrix, int matrix_width, int from, int to, int from2, int to2, int type);
