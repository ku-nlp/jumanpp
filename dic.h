...
class Dic {
  ...
  public:
    ...
    Dic() { posid2pos.get_id(BOS); posid2pos.get_id(EOS); MORPH_DUMMY_POS = posid2pos.get_id(DEF_MORPH_DUMMY); }
    ...
    Node *lookup_lattice(std::vector<CharLattice::da_result_pair_type> &da_search_result, const char *start_str, unsigned int specified_length, unsigned long specified_posid);
    ...
    Node *make_unk_pseudo_node(const char *start_str, int byte_len, unsigned long specified_posid);
    ...
    Node *make_specified_pseudo_node_by_dic_check(const char *start_str, unsigned int specified_length, std::string *inspecified_pos, std::vector<unsigned long> *specified_unk_pos, unsigned int type_family, Node* r_node = nullptr);
