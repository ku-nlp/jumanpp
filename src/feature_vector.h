...
// TODO: 動的読み込みに対応する
// 1. 外から iterator が見えると処理が共通化できない
// 2. 本体はinterface class として，static と dynamic で処理を分岐
...
class FeatureVector {/*{{{*/
  ...
  private:
    ...
    bool create_mmap(std::string mapfile) { //{{{
        // if (debug_mode>0)
        std::cerr << "create mmap file" << std::endl;
        mmap_flag = true;
        ...
        auto weight_map = p_file_weight->construct<Uldmap>("map_weight")(
            0, boost::hash<uint64_t>(), std::equal_to<uint64_t>(),
...
class FeatureVectorIterator
  ...
  private:
    ...
    // 用途無し？
    FeatureVectorIterator(FeatureVector *in_fv, Ulkey uk) {
        fv = in_fv;
        key = uk;
    };
