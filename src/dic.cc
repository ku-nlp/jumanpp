...
Node *Dic::recognize_onomatopoeia(const char *start_str,
    ...
    if (code != TYPE_HIRAGANA && code != TYPE_KATAKANA)
        return nullptr;
    // 小文字で始まる場合は終了
    if (key.is_lower(0))
        return nullptr;
    ...
    for (size_t len = 2; len < 5; len++) { // 反復の長さ
        ...
        if (key.is_choon(len - 1))
            next_code = code; /* 長音は直前の文字種として扱う */
        if (code != next_code)
            break; // カタカナかつ記号，みたいなor を考慮していない
        code = next_code;

        /* 反復があるか判定 */
        if (key.char_substr(0, len) != key.char_substr(len, len))
            continue;
        /* ただし3文字が同じものは不可 */
        ...
        if (specified_length > 0 && (len * 2 * BYTES4CHAR) != specified_length)
            continue;
        ...
        new_node->char_num =
            utf8_chars((unsigned char *)start_str, new_node->length);
        ...
        char *end_char = (char *)get_specified_char_pointer(
            ...
            new_node->char_num - 1);
        new_node->end_char_family =
            check_char_family((unsigned char *)end_char);
        new_node->end_string =
            new std::string(end_char, utf8_bytes((unsigned char *)end_char));
        ...
        //        /* 拗音を含む場合 */
        //        for (i = CONTRACTED_LOWERCASE_S; i < CONTRACTED_LOWERCASE_E;
        //        i++) {
        //            if (strstr(m_buffer[m_buffer_num].midasi, lowercase[i]))
        //            break;
        //        }
        //        if (i < CONTRACTED_LOWERCASE_E) {
        //            if (len == 2) continue; /* 1音の繰り返しは禁止 */
        //            /* 1文字分マイナス+ボーナス */
        //            m_buffer[m_buffer_num].weight -= REPETITION_COST +
        //            CONTRACTED_BONUS;
        //        }
        //        /* 濁音・半濁音を含む場合 */
        //        for (i = 0; *dakuon[i]; i++) {
        //            if (strstr(m_buffer[m_buffer_num].midasi, dakuon[i]))
        //            break;
        //        }
        //        if (*dakuon[i]) {
        ...
        //            */
        //            /* 先頭が濁音の場合はさらにボーナス */
        //            if (!strncmp(m_buffer[m_buffer_num].midasi, dakuon[i],
        //            BYTES4CHAR))
...
Node *Dic::lookup_lattice(
    ...
    const char *start_str, unsigned int specified_length = 0,
    std::string *specified_pos = nullptr) { //{{{
    if (specified_pos)
        return lookup_lattice(da_search_result, start_str, specified_length,
                              posid2pos.get_id(*specified_pos));
    else
        return lookup_lattice(da_search_result, start_str, specified_length,
                              MORPH_DUMMY_POS);
} //}}}
    ...
    for (size_t i = 0; i < num; i++) {
        ...
        for (size_t j = 0; j < size; j++) {
            ...
            if (specified_posid != MORPH_DUMMY_POS &&
                ...
                continue;
            // result_pair の (value(32bit) << 8) + j(最大8bit) が形態素に対する
            // unique な ID になる．
            ...
            new_node->end_char_family =
                check_char_family((unsigned char *)end_char);
            new_node->end_string = new std::string(
                end_char, utf8_bytes((unsigned char *)end_char));

            // 末尾から二文字目を取り出す
            char *lb2_char = nullptr;
            if (new_node->char_num > 2)
                lb2_char = (char *)get_specified_char_pointer(
                    (unsigned char *)start_str, new_node->length,
                    new_node->char_num - 2);
            ...
            if (((get_stat(result_pair[i]) & OPT_PROLONG_DEL_LAST) ||
                ...
                check_utf8_char_type((unsigned char *)start_str) ==
                    TYPE_KATAKANA &&
                check_utf8_char_type((unsigned char *)lb2_char) ==
                    TYPE_KATAKANA) { // カタカナ語である
...
// 品詞情報を詳細に指定し, 辞書引き
Node *Dic::lookup_lattice_specified(
    std::vector<CharLattice::da_result_pair_type> &da_search_result,
    const char *start_str, unsigned int specified_length,
    ...
    for (size_t i = 0; i < num; i++) {
        ...
        for (size_t j = 0; j < size; j++) { // １つでも条件と異なればskipする　
            if ((!modified_word && specified[0].size() > 0 &&
                 specified_readingid != (token + j)->reading_id) ||
                (!modified_word && specified[1].size() > 0 &&
                 specified_baseid != (token + j)->base_id) ||
                (specified[2].size() > 0 &&
                 specified_posid != (token + j)->posid) ||
                (specified[3].size() > 0 &&
                 specified_sposid != (token + j)->spos_id) ||
                (specified[4].size() > 0 &&
                 specified_formtypeid != (token + j)->form_type_id) ||
                (specified[5].size() > 0 &&
                 specified_formid != (token + j)->form_id)) {
...
// make_unk_pseudo_node_list_some_pos_by_dic_check > make_unk_pseudo_node
//                                         > make_unk_pseudo_node_list_some_pos
//                                         > make_unk_pseudo_node
...
Node *Dic::make_unk_pseudo_node(const char *start_str, int byte_len,
    ...
    new_node->char_type =
        check_utf8_char_type((unsigned char *)new_node->surface);
    new_node->char_family = check_char_family(new_node->char_type);
    ...
    // 品詞に関係なく共通の処理
    new_node->char_num =
        utf8_chars((unsigned char *)(new_node->surface), new_node->length);
    char *end_char = (char *)get_specified_char_pointer(
        (unsigned char *)start_str, new_node->length, new_node->char_num - 1);
...
Node *Dic::make_specified_pseudo_node_by_dic_check(
    ...
    if (char_num > 0 && (specified_length == 0 || specified_length == pos)) {
        if (r_node) //辞書が渡されている場合
            return make_unk_pseudo_node_list_some_pos_by_dic_check(
                start_str, pos, specified_posid, specified_unk_pos, r_node);
        else
            return make_unk_pseudo_node_list_some_pos(
                start_str, pos, specified_posid, specified_unk_pos);
    } else {
        return nullptr;
    }
} //}}}
...
Node *Dic::make_unk_pseudo_node_list_some_pos_by_dic_check(
    ...
    if (specified_posid == MORPH_DUMMY_POS) {
        ...
        Node *new_node =
            make_unk_pseudo_node(start_str, byte_len, specified_posid);
...
Node *Dic::make_pseudo_node_list_in_range(const char *start_str,
    ...
    for (unsigned int pos = 0; pos < length;
        ...
        if (((code & (TYPE_HIRAGANA | TYPE_KANJI | TYPE_KATAKANA | TYPE_ALPH)) &
             (last_code &
              (TYPE_HIRAGANA | TYPE_KANJI | TYPE_KATAKANA | TYPE_ALPH))) == 0 &&
            last_code != 0)
            break;

        // figures and alphabets that are processed separately
        if (compare_char_type_in_family(
                check_utf8_char_type((unsigned char *)(start_str + pos)),
                TYPE_FIGURE | TYPE_KANJI_FIGURE | TYPE_ALPH))
            break;
        // stop if it's a punctuation or a space
        else if (pos > 0 &&
                 compare_char_type_in_family(
                     check_utf8_char_type((unsigned char *)(start_str + pos)),
                     TYPE_FAMILY_PUNC | TYPE_FAMILY_SPACE))
            break;
        else if (char_num > max_char_num) // max characters as <UNK>
            break;
        else if (char_num < min_char_num) { // skip while the length is shorter
        ...
        Node *new_node = make_unk_pseudo_node_list_some_pos(
            start_str, pos + utf8_bytes((unsigned char *)(start_str + pos)),
            specified_posid, &(param->unk_pos));
...
const std::unordered_map<std::string, int> Dic::katuyou_type_map{
    ...
    {"動詞性接尾辞ます型", 31},
    {"動詞性接尾辞うる型", 32}}; //}}}
const std::unordered_map<std::string, int> Dic::katuyou_form_map{
    {"母音動詞:語幹", 1}, //{{{
    {"*:*", 0},
    {"母音動詞:基本形", 2},
    {"母音動詞:未然形", 3},
    ...
    {"ナ形容詞特殊:デアル列タ系連用タリ形", 30},
    {"ナ形容詞特殊:デス列基本形", 31},
    {"ナ形容詞特殊:デス列基本推量形", 32},
    {"ナ形容詞特殊:デス列基本省略推量形", 33},
    {"ナ形容詞特殊:デス列タ形", 34},
    {"ナ形容詞特殊:デス列タ系推量形", 35},
    {"ナ形容詞特殊:デス列タ系省略推量形", 36},
    {"ナ形容詞特殊:デス列タ系条件形", 37},
    {"ナ形容詞特殊:デス列タ系連用テ形", 38},
    {"ナ形容詞特殊:デス列タ系連用タリ形", 39},
    {"ナ形容詞特殊:ヤ列基本形", 40},
    {"ナ形容詞特殊:ヤ列基本推量形", 41},
    {"ナ形容詞特殊:ヤ列基本省略推量形", 42},
    {"ナ形容詞特殊:ヤ列タ形", 43},
