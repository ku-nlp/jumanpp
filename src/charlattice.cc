...
int CharLattice::parse(std::string sent) { //{{{
    ...
    for (size_t pos = 0; pos < length; pos += next_pos) {
        ...
        // auto current_char_type = check_utf8_char_type((unsigned char
        // *)current_char.c_str());
        ...
        if (current_char[0] & 0x80) { /* 全角の場合 */
            // cerr << "<3byte char> " ;
            // (ーの文字 || (〜の文字 && (最後の文字である|| 次が記号 ||
            // 次がひらがな))) && ２文字目以降 //jumanの条件
            // (ーの文字 || 〜の文字) && (最後の文字である|| 次が記号 ||
            // 次がひらがな) && ２文字目以降 //旧条件
            // (ーの文字 || 〜の文字 || ３点リーダ)  && ２文字目以降
            // //現在の条件
            if ((current_char == DEF_PROLONG_SYMBOL1 ||
                 current_char == DEF_PROLONG_SYMBOL2 || false) &&
                (pos > 0)) { //長音の母音化
                auto itr = prolonged_map.find(last_char);
                if (itr != prolonged_map.end()) {
                    // 長音を母音に置換してvectorの末尾でconstruct する
                    node_list[CharNum].emplace_back(
                        itr->second, OPT_NORMALIZE | OPT_PROLONG_REPLACE);
                }
                // cerr << "prolong(rep) " ;
            } else { //小文字の大文字化
        ...
        if (next_pos == BYTES4CHAR) { //全角文字
            // cerr << "<3byte> " << "pos:" << pos << " ";
            int pre_char_type =
                (pos > 0) ? check_utf8_char_type(last_char.c_str()) : -1;
            // cerr << "pre_char_type:" << pre_char_type << " ";
            // unsigned char next_chr = sent[pos+next_pos];
            auto post_char_type = check_utf8_char_type(
                sent.substr(pos + next_pos,
                            utf8_bytes((unsigned char *)&sent[pos + next_pos]))
                    .c_str()); /* 文末の場合は0 */
            ...
            if (pre_char_type > 0) {
                // cerr << "<not first>";
                /* 長音記号で,
                 * 直前が削除されたか、直前が平仮名、直前が漢字かつ直後が平仮名
                 */
                if (((pre_is_deleted || pre_char_type == TYPE_KATAKANA ||
                      pre_char_type == TYPE_HIRAGANA ||
                      (pre_char_type == TYPE_KANJI &&
                       post_char_type == TYPE_HIRAGANA)) &&
                     (current_char == DEF_PROLONG_SYMBOL1 ||
                      current_char == DEF_PROLONG_SYMBOL2
                      // || current_char == DEF_ELIPSIS_SYMBOL1 || current_char
                      // == DEF_ELIPSIS_SYMBOL2
                      )) ||
                    /* 直前が削除されていて、現在文字が"っ"、かつ、直後が文末もしくは記号の場合も削除
                       */
                    (pre_is_deleted && current_char == DEF_PROLONG_SYMBOL3 &&
                     (post_char_type == 0 || post_char_type == TYPE_SYMBOL))) {
                    // cerr << "hatsuon del" << endl;
                    node_list[CharNum].emplace_back("", OPT_PROLONG_DEL);
                    next_pre_is_deleted = 1;
                } else {
                    // cerr << "<check2> " << (lower_map.find(last_char)
                    // !=lower_map.end()) << "preisdel:" << pre_is_deleted << "
                    // ";
                    if ((lower_map.find(last_char) != lower_map.end() &&
                         lower_map.find(last_char)->second == current_char) ||
                        (pre_is_deleted &&
                         lower_list.find(current_char) != lower_list.end() &&
...
CharLattice::da_search_one_step(Darts::DoubleArray &da, int left_position,
                                int right_position) { //{{{
    ...
    for (CharNode &left_char_node : (*left_char_node_list)) {
        // cerr << "l:" << left_char_node.chr << "," <<
        // left_char_node.da_node_pos_num << endl; //0はありえない
        for (size_t i = 0; i < left_char_node.da_node_pos_num;
             i++) { /* 現在位置のtrieのノード群から */
            std::vector<CharNode> &right_char_node_list =
                node_list[right_position];
            for (CharNode &right_char_node : right_char_node_list) {
                // cout << " r:" << right_char_node.chr << "," <<
                // right_char_node.da_node_pos_num << endl;
                if (right_char_node.chr[0] == '\0') { /* 削除ノード */
                    if (left_position >= 0) {
                        right_char_node.node_type.push_back(
                            left_char_node.node_type[i] | right_char_node.type |
                            OPT_PROLONG_DEL_LAST); // このタイプが使われるのは何処？
                        ...
                        if (status >
                            ...
                            // cout << "    " << left_char_node.chr << "." <<
                            // right_char_node.chr << "(" << (((unsigned
                            // int)status) >> 8) << "," << (0xff & status) <<
                            // ")"
                            //     << "stat: "<<  (left_char_node.node_type[i] |
                            //     (right_char_node.type & (~OPT_PROLONG_DEL)) |
                            //     OPT_PROLONG_DEL_LAST) << endl;
                            da_result_pair_type result_pair;
                            // da.set_result(&result_pair, status,
                            // (left_char_node.node_type[i] |
                            // (right_char_node.type & (~OPT_PROLONG_DEL)) |
                            // OPT_PROLONG_DEL_LAST));
                            result.emplace_back(da_result_pair_type(
                                status, 0,
                                (left_char_node.node_type[i] |
                                 (right_char_node.type & (~OPT_PROLONG_DEL)) |
                                 OPT_PROLONG_DEL_LAST)));
                    ...
                    // cerr << "  da_normal" << endl;
                    if (!(right_char_node.type & OPT_DEVOICE ||
                          right_char_node.type & OPT_PROLONG_REPLACE) ||
                        (right_char_node.type & OPT_DEVOICE &&
                         left_position <
                             0) || /* 連濁ノードは先頭である必要がある */
                        (right_char_node.type & OPT_PROLONG_REPLACE &&
                         left_position >=
                             0)) { /* 長音置換ノードは先頭以外である必要がある
                                      */
                        // cout << "   da_ind: "<< i <<endl;

                        // current_node_type = left_char_node.node_type[i] |
                        // right_char_node.type;
                        current_da_node_pos = left_char_node.da_node_pos[i];

                        size_t tmp = 0;
                        int status = da.traverse(right_char_node.chr,
                                                 current_da_node_pos, tmp);
                        ...
                        if (status >
                            ...
                            // cout << "    match:" << (status > 0) << endl;
                            // cout << "    " << left_char_node.chr << "." <<
                            // right_char_node.chr << "(" << (status >> 8) <<
                            // "," << (0xff & status) << ")" << " stat:" <<
                            // (left_char_node.node_type[i] |
                            // right_char_node.type) << endl;
                            da_result_pair_type result_pair;
                            // da.set_result(&result_pair, status,
                            // left_char_node.node_type[i] |
                            // right_char_node.type);
                            result.emplace_back(
                                status, 0, left_char_node.node_type[i] |
                                               right_char_node.type); //返り値
                        }

                        // マッチした場合も続きはあるかもしれない
                        if (status > 0 ||
                            status ==
                                -1) { /* マッチした場合と、ここではマッチしていないが、続きがある場合
                                         */
                            // if(status == -1){
                            //    cout << "    continue:" << endl;
                            //}
...
std::vector<CharLattice::da_result_pair_type>
CharLattice::da_search_from_position(Darts::DoubleArray &da,
                                     int position) { //{{{
    std::vector<da_result_pair_type> result;
    if (!constructed)
        return result;
    /* initialization */
    CharLattice::MostDistantPosition =
        position - 1; //並列化の障害にもなるのでローカル変数にできないか？
    ...
    for (size_t i = position + 1; i < node_list.size(); i++) { // 二文字
        ...
        std::vector<da_result_pair_type> tokens =
            da_search_one_step(da, i - 1, i);
