...
void Node::append_bigram_feature(Node *left_node,
                                 FeatureSet *in_feature) { //{{{
    best_bigram[left_node->id]->feature->append_feature(in_feature);
    // left_node->feature = best_bigram[left_node->id]->feature;
    // //とりあえずのコピー 二重deleteされる
} //}}}
...
bool Node::topic_available() { //{{{
    ...
    // 複合辞の処理
    // めぐる 除く のぞく 通じる 通ずる 通す 含める ふくめる 始める はじめる
    // 絡む からむ 沿う そう 向ける 伴う ともなう 基づく よる 対する 関する
    // 代わる おく つく とる 加える くわえる 限る 続く つづく 合わせる あわせる
    // 比べる くらべる 並ぶ ならぶ おける いう する
    int not_hukugou_ji = 1;
    for (int j = 0; j < NUM_OF_FUKUGOUJI; j++) {
        // 代表表記の前半と複合辞が一致しない
        not_hukugou_ji =
            not_hukugou_ji && node->representation &&
            (node->representation->compare(0, strlen(hukugouji[j]),
                                           hukugouji[j], strlen(hukugouji[j])));
    }

    int not_huzoku = (node->semantic_feature) &&
                     (node->semantic_feature->find("付属動詞候補") ==
                      std::string::npos); //付属動詞でない

    int not_keisiki = !((*node->pos == "名詞") &&
                        (*node->spos == "形式名詞")); // 形式名詞でない

    int not_timei_jinmei =
        (!((*node->pos == "名詞") && (*node->spos == "人名")) &&
         !((*node->pos == "名詞") && (*node->spos == "地名")));

    int not_jisou = !((*node->pos == "名詞") &&
                      (*node->spos == "時相名詞")); // 時相名詞でない
    ...
    // 漢字一字，ひらがな一字,
    // 記号にはトピックが関係ない場合が多いはずなので，一文字の形態素はトピックを考えない．
    bool not_one_char = (node->char_num > 1); //表層が日本語1文字以上
    char *result = db_get(
        topic_cdb,
        node->base
            ->c_str()); // TODO:二回トピックを引くのは無駄なので，キャッシュする
    // トピックを読み込む条件
    bool topical_cond = not_one_char && focused_hinsi && not_huzoku &&
                        not_suuryou && not_hukugou_ji && not_keisiki &&
                        not_timei_jinmei && not_jisou && result;
...
void Node::read_vector(const char *buf,
                       std::vector<double> &vector) { // topic用 {{{
...
TopicVector Node::get_topic() { //{{{
    ...
    const char *result =
        db_get(topic_cdb, base->c_str()); // topic ベクトルの読み込み
    if (!result) {
        return node_topic;
    } //トピックが無い場合
