...
std::string WeightedVector::str() {
    if (!canonical)
        canonicalize();
    std::stringstream ss;
    BOOST_FOREACH (Unit x, vec) { ss << x.first << ":" << x.second << " "; }
    return ss.str();
}
...
WeightedVector &WeightedVector::add_with_check(int index, double value) {
    ...
    auto result = std::lower_bound(
        this->vec.begin(), this->vec.end(), std::make_pair(index, value),
...
WeightedVector &WeightedVector::merge(double coef, WeightedVector &r_wvec) {
    ...
    while (!(it_l == l_end && it_r == r_end)) {
        if (it_l == l_end || l_vec[it_l].first > r_vec[it_r].first) {
            this->vec.push_back(
                std::make_pair(r_vec[it_r].first, r_vec[it_r].second * coef));
            it_r++;
...
WeightedVector *WeightedVector::operator+(vector_imp &r_vec) {
    ...
    while (!(it_l == this->vec.end() && it_r == r_vec.end())) {
        if (it_l == this->vec.end() || it_l->first > it_r->first) {
            ...
            tmp->vec.push_back(
                std::make_pair(it_l->first, it_l->second + it_r->second));
            it_l++;
            it_r++;
        }
    }
...
// double kl_divergence(double mean1, double mean2, const WeightedVector&
// variance1, const WeightedVector& variance2) {
//
//}
