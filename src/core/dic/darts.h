...
#define DARTS_THROW(msg)                                         \
  throw JppDarts::Details::Exception(__FILE__ ":" DARTS_LINE_STR \
                                              ": exception: " msg)
...
using progress_func_type = ::jumanpp::core::ProgressCallback *;
...
class Exception : public std::exception {
 public:
  explicit Exception(const char *msg = nullptr) throw() : msg_(msg) {}
...
class BitVector {
 ...
 private:
  ...
  static id_type pop_count(id_type unit) {
    unit = ((unit & 0xAAAAAAAAU) >> 1U) + (unit & 0x55555555U);
    unit = ((unit & 0xCCCCCCCCU) >> 2U) + (unit & 0x33333333U);
    unit = ((unit >> 4U) + unit) & 0x0F0F0F0FU;
    unit += unit >> 8U;
    unit += unit >> 16U;
    return unit & 0xFFU;
  }
...
int DoubleArrayImpl<A, B, T, C>::build(
  ...
  if (progress_func != NULL) {
    progress_func->report(num_keys + 1, num_keys + 1);
  }
