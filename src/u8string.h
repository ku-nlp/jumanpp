 ...
 * 中身の書き換えは非対応
 * コンストラクタでは utf8 の解釈は行わず，単に内部に std::string
 *としてコピーするのみ．
 ...
 * size(), length() は何のサイズを返すべきか曖昧なので, 実装しない．代わりに
 *byte_size, char_size を用意した．
 * morita(2015/01/16)
 ******/
...
class U8string { //{{{
  ...
  public:
    ...
    static constexpr unsigned long KANJI_FIGURE =
        0x00000800; // KANJI+FIGURE にするべき？
    ...
    // TYPE_FIGURE + TYPE_PERIOD + TYPE_MIDDLE_DOT + TYPE_KANJI_FIGURE +
    // TYPE_SLASH + TYPE_COLON
    ...
    // TYPE_PERIOD + TYPE_COMMA + TYPE_IDEOGRAPHIC_PUNC + TYPE_MIDDLE_DOT +
    ...
    static constexpr unsigned long FAMILY_KANJI =
        0x0000080A; // TYPE_KANJI + TYPE_KANJI_FIGURE
    ...
    static const std::unordered_set<unsigned int>
        brackets; //あとでmap にするかも
  ...
  public:
    static size_t character_length(const std::string &c_str) { /*{{{*/
        size_t clen = 0;
        for (size_t i = 0, byte = utf8_bytes(c_str[0]); i < c_str.size();
             byte = utf8_bytes(c_str[i]), i += byte) {
            ++clen;
        }
        return clen;
    } /*}}}*/
    ...
    inline void parse() { //{{{
        ...
        char_array.clear();
        char_index2byte_index.clear();
        for (size_t i = 0, byte = utf8_bytes(byte_str[0]); i < byte_size();
             byte = utf8_bytes(byte_str[i]), i += byte) {
    ...
    std::string operator[](size_t char_pos) { //{{{
        parse();
        return (
            byte_str.substr(in_byte_index(char_pos), byte_size_at(char_pos)));
    }; //}}}
    ...
    std::string char_substr(size_t char_pos, size_t char_length) { //{{{
        parse();
        return (byte_str.substr(in_byte_index(char_pos),
                                in_byte_index(char_pos + char_length) -
                                    in_byte_index(char_pos)));
    }; //}}}
    ...
    unsigned long char_type_at(size_t char_ind) { //{{{
        ...
        //            }
        //            std::cerr << " " << std::hex <<
        //            to_unicode(char_array[char_ind]) << std::dec << std::endl;

        if (char_ind >= char_size())
            return 0x0000;
  ...
  private:
    ...
    int prev(int pos) { //{{{
        ...
        while ((0x80 <= static_cast<unsigned char>(byte_str[tmp])) &
               (static_cast<unsigned char>(byte_str[tmp]) <
                0xC0)) { // 2byte 以降はこの範囲に入る
            tmp--;
            if (tmp < 0)
                return -1;
        }
